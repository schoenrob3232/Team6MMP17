...
void acquireTestTrainingData(Mat &labels_arg, Mat &data_arg)
{
	...
	while (getline(filePos, line) && i < 5000) 
	{
		...
		if (!sampleImg.empty()) 
		{
			acquireSimplePositive(sampleImg, labels_arg, data_arg);
			i++;
		}
	}
	...
	while (getline(fileNeg, line) && i < 500) 
	{
		...
		if (!sampleImg.empty()) 
		{
			acquireRandomNegatives(sampleImg, labels_arg, data_arg, 10);
			i++;
		}
	}
}
...
/*
	Acquires the hardest negative on the Mat-Image img_arg which is misdetected by svm_name, and adds it to 
...
/*
	uses acquireHardestNegative to look through the whole negatives-list for hard negatives and
...
void acquireMultipleHardNegatives(const char* svm_name, Mat &labels_arg, Mat &data_arg) 
{
	...
	while (getline(fileNeg, line))
	{
		...
		if (!sampleImg.empty()) 
		{
			acquireHardestNegative(sampleImg, svm_name, labels_arg, data_arg, groundTruth);
		}
	}
...
/*
	Acquires positive Samples according to annotations
*/
...
void acquireUltraHardNegatives(const char* svm_name, Mat &labels_arg, Mat &data_arg) 
{
	...
	while (getline(filePos, line) && getline(fileAnnotations, annotation_line)) 
	{
		...
		{
					acquireHardestNegative(sampleImg, svm_name, labels_arg, data_arg, annotation);
		}
	}
